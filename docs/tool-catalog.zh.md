...
### `present_piece_for_approval`

Present a specification piece for an explicit Accept, Question, or Change decision. Only a live root agent may ask. Acceptance queues an unchanged todo piece; questions and changes do not queue work.
...
```json
{
  ...
  "properties": {
    "pieceId": {
      "type": "string",
      "description": "Specification piece id."
    }
  },
  "required": [
    "pieceId"
...
Source: [`packages/dev-loop/approval/src/index.ts`](../packages/dev-loop/approval/src/index.ts)

The opt-in consumer presents current source to a live root caller. Accept changes lifecycle memory to pending; it does not dispatch work or persist an approval grant.
