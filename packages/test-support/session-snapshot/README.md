...
Web command-only scenarios may declare `input.operations`: `{ kind: command, run: 0 }` selects a zero-based canonical `command/run` record, and `{ kind: restart }` closes the process before relaunching the same private home and reattaching the same Session identity and cwd. Command ordinals must cover the canonical log exactly once in ascending order; restarts must separate commands. Unknown fields, skipped or duplicate ordinals, and mixed task/attachment input fail validation. Command text remains solely in Session JSONL.
...
| [`src/command-operations.ts`](src/command-operations.ts) | Closed Web command-ordinal and restart validation |
