...
export interface TestSessionRemote {
  ...
  subagentModelCatalog(signal?: AbortSignal): Promise<RemoteResult<ModelCatalog>>
...
export function createSessionTestRemote(
  ...
  return {
    ...
    subagentModelCatalog: (signal = new AbortController().signal) => remoteResult(
      () => direct.subagentModelCatalog(signal), signal,
    ),
