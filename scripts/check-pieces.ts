...
import type { FenceMarker, PieceFinding, PieceRecord } from './parse-piece.ts'
...
export function checkClaimCitations(path: string, content: string): Violation[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (start === -1) {
      if (line === '## Resources and proof') start = i
    } else {
      if (line.startsWith('## ')) {
        end = i
        break
  ...
  for (const row of data) {
    ...
    const citation = (cells[1] ?? '').trim()
    if (!citation) {
      violations.push({ path, message: `proof row does not state a citation: ${row.trim().slice(0, 90)}` })
    }
    const established = (cells[2] ?? '').trim().toLowerCase()
    const isValid = ESTABLISHED.some((form) => {
      if (form === 'reported by' || form === 'unverified') {
        return established.startsWith(form)
      }
      return form === established
    })
    if (!isValid) {
...
export function checkSetIndex(path: string, id: string, read: IndexReader = readIndex): Violation[] {
  ...
  const matchedRow = rows.find((line) => {
    ...
    return cells[0] === `\`${id}\`` || cells[0] === id
  })
  if (matchedRow === undefined) {
    return [{ path, message: `is done but ${relative(process.cwd(), readme)} lists no row for \`${id}\`` }]
  }
  const cells = tableCells(matchedRow)
  const statusCell = (cells[cells.length - 1] ?? '').trim().toLowerCase()
  if (statusCell !== 'done') {
...
const NON_CREATING_BRANCH_FLAGS = new RegExp(
  '^(?:-[dDamrMlv]+|--(?:delete|all|remotes|move|list|show-current|verbose|merged|no-merged|' +
    'contains|no-contains|edit-description|help))$',
)
...
export function findBranchCreation(snippet: string): string | undefined {
  ...
  while ((match = gitPattern.exec(snippet)) !== null) {
    ...
    if (subcommand === 'checkout') {
      if (/(?:^|\s)(?:-[bB]|--branch|--orphan)(?:\s|$)/.test(args)) return match[0].trim()
    } else if (subcommand === 'switch') {
      if (/(?:^|\s)(?:-[cC]|--create|--orphan)(?:\s|$)/.test(args)) return match[0].trim()
...
if (import.meta.filename === resolve(process.argv[1] ?? '')) {
  ...
  const count = targetCount(files)
  if (files.length > 0 && count === 0) {
    console.error(`check-pieces ${name}: no valid piece files found in arguments (${files.join(', ')})`)
    process.exitCode = 1
  } else {
    ...
    if (violations.length > 0) {
      ...
      console.log(`check-pieces ${name}: ${String(count)} piece(s) conform.`)
    }
  }
}
